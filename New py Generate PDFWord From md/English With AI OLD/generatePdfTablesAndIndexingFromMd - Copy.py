...
def extract_headings(markdown_content):
    """Extract H1-H5 headings and assign hierarchical numbering."""
    headings = []
    counters = [0, 0, 0, 0, 0]
    ...
    for line in markdown_content.splitlines():
        ...
        number = ".".join(str(counters[j]) for j in range(level))
        headings.append({
            "level": level,
            "number": number,
            "title": title,
...
def add_numbering_to_html(html_content):
    ...
    def replace_heading(match):
        ...
        number = ".".join(str(counters[j]) for j in range(index + 1))
...
def build_toc_html(headings, title):
    ...
    If the document has no headings at all, return an empty string so
    build_document_html() skips the TOC page entirely instead of
    inserting a blank "no sections found" page -- the document itself
    still gets converted in full either way.
    """
    if not headings:
        return ""
    ...
    for h in headings:
        ...
        items.append(
            f'<div class="toc-item toc-level-{h["level"]}" '
            f'data-target="{h["id"]}" style="--toc-indent:{indent}">'
            f'<span class="toc-number">{h["number"]}</span>'
            f'<span class="toc-title">{format_text(h["title"])}</span>'
            f'<span class="toc-dots"></span>'
            f'<span class="toc-page-number">?</span>'
            f'</div>'
        )
