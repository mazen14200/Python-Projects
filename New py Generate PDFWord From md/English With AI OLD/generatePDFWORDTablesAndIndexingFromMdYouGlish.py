...
def add_rich_paragraph(doc, text: str, *, style=None, alignment=WD_ALIGN_PARAGRAPH.RIGHT, hyperlink_url: Optional[str] = None):
    ...
    for kind, value, url in parse_inline_segments(text):
        if kind == 'text':
            ...
            color = english_color(value) if is_english_only_text(value) else '2980B9'
            add_external_hyperlink(p, value, url, color=color, underline=False, bold=is_english_only_text(value))
        elif kind == 'code':
            r = p.add_run(value)
            ...
            for kind, val, url in parse_inline_segments(value):
                if kind == 'text':
                    ...
                    link_color = 'FFFFFF' if ri == 0 else (
                        ...
                        else (english_color(val) if is_english_only_text(val) else '2980B9')
                    )
                    add_external_hyperlink(
                        p,
                        val,
            ...
            if doc.paragraphs:
                ...
                if visible.strip():
                    ...
                    add_external_hyperlink_before(
                        ...
                        bold=is_english_only_text(visible)
                    )
                else:
