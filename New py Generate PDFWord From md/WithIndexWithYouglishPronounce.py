...
def add_pronounce_hyperlink(paragraph, source_text: str, *, size=PRONOUNCE_SIZE,
    ...
    """Append a small clickable 'pronounce' link for an English paragraph."""
    url = build_pronounce_url(source_text)
    if not url:
        return False

    # Avoid duplicate pronounce links in the same paragraph.
    paragraph_text = ''.join(r.text or '' for r in paragraph.runs)
    if PRONOUNCE_TEXT.lower() in paragraph_text.lower():
        return False

    # Keep a small visual separation without creating a new line.
    spacer = paragraph.add_run(PRONOUNCE_SEPARATOR)
    set_run_font(spacer, size=size, color=color)
...
def add_pronounce_after_existing_hyperlink(paragraph, source_text: str, *, size=PRONOUNCE_SIZE,
                                            color=PRONOUNCE_COLOR, bold=False) -> bool:
    """Append pronounce after an existing hyperlink-containing paragraph."""
    return add_pronounce_hyperlink(paragraph, source_text, size=size, color=color, bold=bold)
...
def add_rich_paragraph(doc, text: str, *, style=None, alignment=WD_ALIGN_PARAGRAPH.RIGHT, hyperlink_url: Optional[str] = None):
    ...
    add_pronounce_hyperlink(p, text)
    return p
...
def add_table(doc, rows, table_index):
    ...
    for ri, row in enumerate(body_rows):
        for ci in range(cols):
            ...
            add_pronounce_hyperlink(
                p,
                value,
                size=PRONOUNCE_TABLE_SIZE,
                color='FFFFFF' if ri == 0 else PRONOUNCE_COLOR,
                bold=False
            )
...
def build_document(markdown_content, source_name):
    ...
    # First page: document title + (when available) TOC.
    if headings:
        add_toc_page(doc, title, headings)

        # Force content into a new section so its PAGE number starts at 1.
    ...
    while i < n:
        ...
        if hm:
            ...
            add_pronounce_hyperlink(p, text, size=8.0, color=PRONOUNCE_COLOR, bold=False)
            i += 1
            continue
        ...
        if re.search(r'=\s*https?://[^\s]+', stripped, re.IGNORECASE) or re.search(r'=https?://[^\s]+', stripped, re.IGNORECASE):
            ...
            if url_matches:
                ...
                add_pronounce_hyperlink(p, stripped)
                i += 1
                continue
...
def extract_page_numbers_from_pdf(pdf_path, headings):
    """
    Extract the *logical page numbers printed in the PDF TOC page column*.

    Important detail for RTL Word-generated PDFs:
    PyMuPDF's logical table-column order is not reliable.  Instead of taking
    a value from table_data[row][page_idx] or from a physical-page fallback,
    this function:
      1) finds the real physical x-range of the TOC "الصفحة" column from the
         TOC header geometry;
      2) finds each TOC section number by its exact visible number token;
      3) reads only numeric text inside that physical page-column x-range on
         the same horizontal line.

    This also handles a TOC that continues onto page 2 without repeating the
    header row.  In the supplied PDF, for example, 0.19 and 0.20 are on page 2
    and both have the visible logical page number 11.
    """
    ...
    try:
        ...
        # ------------------------------------------------------------
        # 1) Find the first real TOC table and its physical page-column x-range.
        # ------------------------------------------------------------
        page_column_bbox = None
        toc_start_page = None
        scan_limit = min(len(pdf), 4)
        ...
        for page_num in range(scan_limit):
            ...
            for table in tables:
                ...
                header = data[0]
                number_idx, title_idx, page_idx = _detect_toc_columns(header)
                if page_idx is None or (number_idx is None and title_idx is None):
                    continue

                bbox = _find_toc_page_column_bbox(page, table, page_idx)
                if bbox:
                    page_column_bbox = bbox
                    toc_start_page = page_num
                    break
            if page_column_bbox:
                break

        if not page_column_bbox:
            pdf.close()
            return {}

        page_col = fitz.Rect(page_column_bbox)
        page_x0, page_x1 = page_col.x0, page_col.x1

        # ------------------------------------------------------------
        # 2) Build exact heading-number -> heading mapping.
        # ------------------------------------------------------------
        heading_by_number = {}
        for heading in headings:
            key = _compact_number_text(heading.get('number', ''))
            if key:
                heading_by_number.setdefault(key, []).append(heading)

        # We search only the TOC pages starting at the first TOC page.
        # Stop early once every heading has been matched.
        max_toc_pages = min(len(pdf), toc_start_page + 4)

        for page_num in range(toc_start_page, max_toc_pages):
            page = pdf[page_num]
            words = page.get_text('words', sort=True) or []
            if not words:
                continue

            # Exact visible section-number tokens on this page.
            heading_tokens = []
            for w in words:
                if len(w) < 5:
                    continue
                token = _compact_number_text(w[4])
                if token in heading_by_number:
                    heading_tokens.append(w)

            if not heading_tokens:
                continue

            # Numeric tokens in the *physical page column only*.
            page_tokens = []
            for w in words:
                if len(w) < 5:
                    continue
                ...
                if not re.fullmatch(r'\d{1,4}', norm or ''):
                    continue
                cx = (float(w[0]) + float(w[2])) / 2.0
                if page_x0 - 2 <= cx <= page_x1 + 2:
                    try:
                        value = int(norm)
                    except ValueError:
                        continue
                    if 0 <= value <= 9999:
                        page_tokens.append((w, value))

            for heading_word in heading_tokens:
                number_key = _compact_number_text(heading_word[4])
                candidates = heading_by_number.get(number_key, [])
                if not candidates:
                    continue

                # A heading number is unique in the supplied document.  If a
                # duplicate exists, assign the first still-unmatched heading.
                heading = next(
                    (h for h in candidates if h['bookmark'] not in page_numbers),
                    None
                )
                if heading is None:
                    continue

                heading_y = (float(heading_word[1]) + float(heading_word[3])) / 2.0
                heading_h = max(1.0, float(heading_word[3]) - float(heading_word[1]))

                # Prefer a page-column number on the same horizontal line.
                ranked = sorted(
                    page_tokens,
                    key=lambda item: abs(((float(item[0][1]) + float(item[0][3])) / 2.0) - heading_y)
                )

                page_number = None
                best_delta = None
                for word, value in ranked:
                    y = (float(word[1]) + float(word[3])) / 2.0
                    delta = abs(y - heading_y)
                    # Same-row tolerance; slightly generous for wrapped RTL
                    # rows and Word/PDF glyph positioning.
                    if delta <= max(7.0, heading_h * 1.75):
                        page_number = value
                        best_delta = delta
                        break
            ...
            if len(page_numbers) == len(headings):
                break
...
def process_file(input_file):
    ...
    if pdf_result:
        ...
        # --------------------------------------------------------
        # Extract page numbers from PDF and update Word TOC
        # --------------------------------------------------------
        page_numbers = extract_page_numbers_from_pdf(output_pdf, headings)

        if page_numbers:
            update_result = update_word_toc_with_page_numbers(output_docx, page_numbers, headings)
            if update_result:
                print('  Word TOC updated with actual page numbers from PDF')
            else:
                print('  WARNING: Failed to update Word TOC')
        else:
            print('  WARNING: No page numbers extracted from PDF')
